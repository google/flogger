...
/**
 * A message formatter that omits the context data.
 */
final class Log4j2MessageFormatter extends MessageBuilder<StringBuilder>
     ...
     *
     * Instead of creating a standard Flogger message, e.g. ... Foo='bar' [CONTEXT count=23 id="test ID" ],
     * we omit displaying both context map data and context stack data and create a simple minimal
     * message only, e.g. ... Foo='bar'.
     *
     * The simple message together with both context stack and context map data is propagated to Log4j2.
     */
        ...
        switch (format) {
            ...
            case CHAR:
                if (options.isDefault()) {
                    ...
                    if (codePoint >>> 16 == 0) { // see: Character.isBmpCodePoint since Java 1.7
